...
class PagesController extends Controller
{
    public function getHome(){
    	$mobile=DB::table('products')->where('cat_id',1)->join('product_details','product_details.pro_id','=','products.id')->select('products.*','product_details.vga','product_details.screen','product_details.cpu','product_details.cam2')->paginate(8);
	    $pc=DB::table('products')->where('cat_id',7)->join('product_details','product_details.pro_id','=','products.id')->select('products.*','product_details.vga','product_details.screen','product_details.cpu','product_details.cam2')->paginate(8);
    ...
    public function Cart(){
    	...
    	$total=Cart::total();
        return view('front-end.cart',compact(['content','total']));
    }
    ...
    public function checkOut(){
	    $content=Cart::content();
	    $total=Cart::total();
    	return view('front-end.checkout',compact(['content','total']));
    }
}
